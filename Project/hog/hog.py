...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    if player_score%10 != opponent_score//10%10 :
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if num_factors(score) == 3:
        while not is_prime(score+1):
            score = score+1
        return score + 1
    elif num_factors(score) == 4:
        while not is_prime(score+1):
            score = score+1
        return score + 1
    else:
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while n <=10 :
        score = average_score(n,dice)   #易犯错误：将函数直接当变量使用，这是错误的
        if score>maximum:
            maximum = score
            result = n
        n = n + 1   
