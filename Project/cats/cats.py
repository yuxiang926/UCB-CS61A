...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    right = [paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])]
    if k>=len(right):
        return ''
    return right[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def match(paragraph):
        about_paragraph = split(remove_punctuation(paragraph))#生成一个拆为单词的列表
        for i in range(len(about_paragraph)):
            for n in range(len(subject)):
                if lower(about_paragraph[i]) == lower(subject[n]):
                    return True
        return False
    return match
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        min_score = diff_function(typed_word,word_list[0],limit)
        index_min_word = 0
        for i in range(len(word_list)):
            if diff_function(typed_word,word_list[i],limit)<min_score:
                min_score = diff_function(typed_word,word_list[i],limit)
                index_min_word = i
        if min_score <= limit:
            return word_list[index_min_word]
        else:
            return typed_word 
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def equal(typed,source, limit,flag):
        if typed == ''or source == '':
            return 0
        elif flag>limit:
            return limit
            ...
            return equal(typed[1:],source[1:],limit,flag) + 1
        else:
            return equal(typed[1:],source[1:],limit,flag)
    return equal(typed,source,limit,0)+abs(len(typed)-len(source))
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        times = times+[[]]
        for k in range(1,len(tpp[i])):
            times[i] = times[i] + [tpp[i][k]-tpp[i][k-1]]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for k in word_indices:
        ...
        for i in player_indices:
            flag = smaller
            smaller = min(smaller,times[i][k])
            if smaller!= flag:
                index = i
        
        fastest_result[index] = fastest_result[index] + [words[k]] 
