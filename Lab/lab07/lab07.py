class Account:
    ...
    def time_to_retire(self, amount):
        ...
        result = 1
        while True:
            if self.balance*pow(self.interest+1,result)>=amount:
                return result
            result += 1
...
def without(s, i):
    ...
    "*** YOUR CODE HERE ***"
    index = 0
    def helper(i,s,index):
        if s is Link.empty:
            return s
        if index == i:
            index += 1
            return Link(s.rest.first,helper(i,s.rest.rest,index))
        else:
            index+=1
            return Link(s.first,helper(i,s.rest,index)) 
    return helper(i,s,index)
